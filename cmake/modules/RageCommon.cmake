...
cmake_dependent_option (RAGE3D_64BIT "Enable 64-bit build, the default is set based on the native ABI of the chosen compiler toolchain" "${NATIVE_64BIT}" "NOT MSVC AND NOT ANDROID AND NOT (ARM AND NOT IOS) AND NOT WEB AND NOT POWERPC" "${NATIVE_64BIT}")     # Intentionally only enable the option for iOS but not for tvOS as the latter is 64-bit only
option (RAGE3D_ANGELSCRIPT "Enable AngelScript scripting support" TRUE)
option (RAGE3D_IK "Enable inverse kinematics support" TRUE)
option (RAGE3D_LUA "Enable additional Lua scripting support" TRUE)
option (RAGE3D_NAVIGATION "Enable navigation support" TRUE)
cmake_dependent_option (RAGE3D_NETWORK "Enable networking support" TRUE "NOT WEB" FALSE)
option (RAGE3D_PHYSICS "Enable physics support" TRUE)
option (RAGE3D_RAGE2D "Enable 2D graphics and physics support" TRUE)
option (RAGE3D_WEBP "Enable WebP support" TRUE)
...
if ((RPI AND "${RPI_ABI}" MATCHES NEON) OR (ARM AND (APPLE OR RAGE3D_64BIT OR "${ARM_ABI_FLAGS}" MATCHES neon)))    # Stringify in case RPI_ABI/ARM_ABI_FLAGS is not set explicitly
