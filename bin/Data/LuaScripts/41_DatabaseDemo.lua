...
function Start()
    ...
    -- The ODBC API, i.e. RAGE3D_DATABASE_ODBC build option, is only available for native (including RPI) platforms
    --   and it is designed for development of game server connecting to ODBC-compliant databases in mind

    -- This demo will always work when using SQLite API as the SQLite database engine is embedded inside Rage3D game engine
    ...
    -- The "RAGE3D_DATABASE_ODBC" compiler define is set when RAGE3D_DATABASE_ODBC build option is enabled
