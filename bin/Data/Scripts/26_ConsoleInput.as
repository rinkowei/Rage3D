...
// Rage threat level descriptions
String[] urhoThreatLevels = {
    "Suddenly Rage appears from a dark corner of the fish tank",
    "Rage seems to have his eyes set on you",
    "Rage is homing in on you mercilessly"
};
...
void StartGame()
{
    Print("Welcome to the Rage adventure game! You are the newest fish in the tank; your\n"
          "objective is to survive as long as possible. Beware of hunger and the merciless\n"
          "predator cichlid Rage, who appears from time to time. Evading Rage is easier\n"
    ...
    else if (gameOn)
    {
        ...
        else if (inputLower == "hide")
        {
            if (urhoThreat > 0)
            {
                ...
                if (evadeSuccess)
                {
                    Print("You hide behind the thick bottom vegetation, until Rage grows bored.");
                    urhoThreat = -2;
                }
                else
                    Print("Your movements are too slow; you are unable to hide from Rage.");
            }
            else
